...
class HeadlineTab extends StatefulWidget {
  const HeadlineTab({ Key? key }) : super(key: key);

  @override
  _HeadlineTabState createState() => _HeadlineTabState();
}

class _HeadlineTabState extends State<HeadlineTab> {
  ...
  Future getNews() async {
    ...
    try {
      isLoading = true;
      response = await _networkService.getRequest("/top-headlines?sources=techcrunch&apiKey=b68e68a90afd4125a8edba60c536822c");
      isLoading = false;
      if(response.statusCode == 200) {
        setState(() {
          _headline = Headline.fromJson(response.data);
          _newsList = _headline.articles;
        });
      }else{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      ) : _newsList != null ? ListView.builder(
